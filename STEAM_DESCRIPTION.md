...
[*] [b]多目標行程[/b]：最多16點，可加尾、插入、優先、排序／預覽。新行程停妥後自動接續；停靠／逐點等你按「繼續自駕」（舊行程維持逐點）。未到目標不算抵達、不跳站。需MiniMap0.28.0+、AutoDrive0.8.0+，更新後完整重啟
[*] [b]駕駛HUD[/b]：狀態、目前／下一目標、車速、巡航上限、檔位、減速、電油量；可啟停、切換共用自動／逐點模式與語音。金屬／玻璃／家族／側翼四主題，支援精簡與收合
...
[*] [b]單人自動暫停[/b]：脫困失敗與停靠／最後完成可各自設定（預設開）；普通自動接續點不暫停。停妥後播完語音才暫停，語音關閉／不可用則直接暫停；新駕駛意圖取消待執行暫停。只限單人，暫時堵路、手動停止、多人／Host、分割畫面除外。解除暫停不自動發車，語音期間世界仍運作
