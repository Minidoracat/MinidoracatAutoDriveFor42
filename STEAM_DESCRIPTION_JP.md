...
[h2]🧰 主な機能[/h2]
[list]
[*] [b]車載装置[/b]：対応車両にGPS・自動運転モジュールの部品枠を追加。取り外しても電量・追加データを保持
[*] [b]複数目的地[/b]：最大16点、追加・挿入・優先・並替・経路表示。新規は停車後に継続、停留／逐点は操作待ち。未到達の道路終点で完了・スキップしません
[*] [b]運転HUD[/b]：状態・現在／次の目的地・速度・上限・ギア・減速・電油量、運転・共通の自動／逐点モード・音声操作、ワンクリック帰宅（ミニマップの自宅）。メタル／ガラス／ファミリー／側翼、縮小・折畳対応
...
[*] [b]シングル一時停止[/b]：脱出失敗／停留・最終到着を個別設定（既定オン）、停車・音声終了後に停止。通常の継続地点・手動停止・マルチ・分割は対象外、解除後も自動発車なし
...
[*] [b]サーバー診断収集[/b]：サーバー管理者がサンドボックスで有効化（既定オフ）。問題発生時の前後の走行データと走行ごとの概要をサーバーへ送り、自動運転の改善に役立てます。メモリのみでディスクには保存せず、MOD Optionsで参加を拒否できます
