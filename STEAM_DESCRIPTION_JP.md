...
[*] [b]オートパイロットモジュール[/b]：車両に取り付けるとルートに沿って自動運転、目的地到着で自動停車
[*] [b]クラフト対応[/b]：電気スキルで両デバイスを作成可能（サンドボックスでクラフト可否・アイテム要求を切替）
[*] [b]電力消費[/b]：携帯時は本体バッテリー、車載後は車両バッテリーから給電；消費率はサンドボックスで調整可（0 = 消費なし）
[*] [b]パフォーマンス優先[/b]：ルート速度プロファイルの事前計算とイベント駆動の回避で毎フレームコストを最小化
