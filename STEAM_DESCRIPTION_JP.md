...
[h2]✨ これは何？[/h2]
GPSで経路を計画し、モジュールが道路網を自動走行。目的地で停車します。
...
[*] 車の横で[b]右クリックまたは車両整備画面[/b]から取り付け（ドライバー・車用バッテリー・電気1）。GPSは電池入りで所持しても使用可
...
[h2]🧰 主な機能[/h2]
[list]
[*] [b]車載装置[/b]：対応車両にGPS・自動運転モジュールの部品枠を追加。取り外しても電量・追加データを保持。旧車は読込時に自動移行し、競合時は旧記録を保持。更新前にセーブをバックアップし、ゲーム・サーバーを再起動してください
