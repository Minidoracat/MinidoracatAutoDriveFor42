...
[list]
[*] 追加・変更道路には、マップ作者か互換パッチ作者が提供・維持する[b]streets.xml[/b]が必要です。現行ゲームの公式形式、実際の路面との一致、マップ内外の道路接続が条件です。
[*] プレイ可能・地図画像あり・ファイルありだけでは道路データの完全性を保証しません。画像からの道路生成や、欠落・位置ずれ・未接続の自動修復はしません。
[*] 特定マップだけ不調ならMiniMapとAutoDriveを更新し、作者に道路データを確認してください。マップのリンクと診断ログ付きの報告も歓迎します。
[/list]
...
[*] [b]運転HUD[/b]：状態・速度・巡航上限・ギア・減速理由・電力・燃料を表示。開始／停止、ギア、テーマ、表示、音声・音量をHUDで操作。メタル／ガラス／ファミリーカード／サイドウィングの4テーマと縮小・折畳表示。サイドウィングは路面を隠さず左右別に折りたためます
[*] [b]走行時間[/b]：待機・脱出を含む実時間。停止後は前回の時間を表示し、次の開始成功時にリセット。ゲーム実行中のみ保持し、セーブには保存しません
[*] [b]速度ギアと積極的なMAX[/b]：30／50／70 km/hは快適走行。MAXはカーブや隙間を速めに走り、アクセルオフも遅め。HUDで直接切替え、別設定は不要。巡航上限は車両・サンドボックス設定に従い、車況・衝突防止の判定は維持
[*] [b]いつでも手動へ[/b]：ハンドル・アクセル・ブレーキで自動運転を停止し、目的地は保持。パネルから再開できます。「手動介入後」で2／3／5／10秒を選ぶと、手を離した後にHUDのカウントダウンと通知を経て自動再開
...
[*] [b]復旧と迂回[/b]：詰まると別の隙間や後退を試し、通れなければ停車・待機。HUD「迂回」で代替ルートを要求し、設定で自動迂回も可能。解決できなければ通知して操作を返します
...
[*] [url=https://github.com/Minidoracat/MinidoracatAutoDriveFor42/issues/new/choose]GitHub ナビ問題報告[/url]：道間違い・詰まり・突然停止はIssueへ。診断ログ出力をオンにし、Telemetry全体をzipで添付。設定の「ナビの問題を報告」からリンクをコピーできます
[/list]

[h2]☕ 作者を応援[/h2]
このMODは今後も無料です。気に入ったらコーヒーを一杯おごってください。支援はサーバーとMOD開発に使います。ソースはGitHubで公開しています。
