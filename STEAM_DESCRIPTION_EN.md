...
[*] [b]Driver HUD[/b]: shows autodrive state, speed, cruise limit, gear, slowdown reason, power and fuel; start/stop or change gear directly. Four themes (metal, glass, family card, side wings - two panels beside the dashboard that never cover the road, each foldable), plus compact and collapsed modes; style, hide, voice and volume are one click on the HUD itself
[*] [b]Speed gears and Insane mode[/b]: switch between 30 / 50 / 70 km/h and Insane (vehicle max speed). For the initial release, Insane deliberately shares the stable path-following, cornering and obstacle-safety logic, so its main difference is the cruise cap; a future update will rework it into a more aggressive, genuinely wild driving style
[*] [b]Take over anytime[/b]: steering, throttle or braking immediately returns control to the player and switches autodrive off (the navigation target is kept; press the panel again to continue). Prefer it to resume by itself? Pick 2 / 3 / 5 / 10 s under "After manual input" in the options - the HUD counts down while standing by and you get a prompt before it takes over
[*] [b]U-turn style[/b]: when the target moves behind the vehicle, the default "Gentle" slows to a near stop and pivots slowly; switch to "Fast" in the options to swing around with momentum
...
[*] [b]Risk-aware slowdown[/b]: adjusts speed for bends, heading error, zombies, corpses, moving vehicles and unloaded areas; HUD tooltips explain the active policy
