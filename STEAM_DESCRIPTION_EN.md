...
[h2]✨ What is this[/h2]
GPS plans road routes; the Autopilot Module drives them and stops on arrival.
...
[*] Beside the vehicle, install via [b]right-click or Vehicle Mechanics[/b] (screwdriver, vehicle battery and Electrical 1 required). GPS also works from your inventory with a battery
...
[h2]🧰 Features[/h2]
[list]
[*] [b]Vehicle devices[/b]: GPS and Autopilot occupy real parts on supported vehicles. Removal keeps charge and item data
[*] [b]Multi-target trips[/b]: up to 16 targets; append, insert, prioritize, reorder/preview. New trips auto-continue after stopping; stopovers/step mode wait for Continue driving. Road ends short of the target never count as arrival or skip it
[*] [b]Driver HUD[/b]: status, current/next target, speed, cruise cap, gear, slowdown, power/fuel; driving, shared Auto/Step mode, voice and one-click Go home (home set in MiniMap). Metal/glass/family/side-wing themes; compact/collapsed layouts
...
[*] [b]Solo auto-pause[/b]: separate settings for failed recovery and stopovers/final arrival (on by default); pauses after stopping and the voice line. Ordinary targets, manual stops, multiplayer and split-screen never pause; unpausing never restarts driving
...
[*] [b]Server diagnostics[/b]: server owners can enable it in sandbox (off by default). When something goes wrong, a short clip of driving data and a per-drive summary are sent to the server to help improve autodrive; kept in memory only, never written to players' disks. Players can opt out in MOD Options
