...
- 使用螺絲起子，對車右鍵或從維修面板安裝／卸載 GPS 與自駕裝置；兩個入口共用標準零件動作，實際拆裝保留伺服器權威驗證
- 裝置以實體零件隨車保存，卸下保留同一物品的電量與附加資料。舊版已安裝裝置在車輛載入時自動遷移；缺少安全的零件位置或發生衝突時保留舊資料，不覆寫其他物品。升級前請備份存檔
...
- **多目標行程**：搭配 MiniMap nav API v7，共用一份最多 16 點的清單與自動／逐點模式；
  普通自動點停妥後重建下一段再接管，標成停靠的點與逐點模式均等待「繼續自駕」。
  新行程預設自動、舊行程保留手動。HUD 顯示目前／下一目標，並有圖標＋「自動／逐點」模式鈕。
  準備中可取消；無路或逾時保留目標並提示，不跳站。道路未通到指定座標時提示手動前往；
  重新進入存檔不恢復自駕。MiniMap v6 僅逐點手動，更舊版本保留原單段模式。
  多站自駕需 MiniMap `42.20.4-0.28.0+` 與 AutoDrive `42.20.4-0.8.0+`，更新後請完整重啟遊戲。
- **一鍵回家**：搭配 MiniMap nav API v8，HUD 主鈕左側多一顆「回家」（有 UI 框架時為房屋圖示），
  把行程換成小地圖設定的家；單站自駕中直接改往家，停著時只換目標、不發車。未設家時按鈕較暗，
  按下提示設定方法；多站自駕中需先停止。舊版 MiniMap 不顯示此按鈕；與 HUD 其他按鈕相同只支援滑鼠。
- **行程語音**：新增中英日「接續下一點」「已到停靠點」「先去優先目標」三種事件，
  只有真正完成最後目標才播原抵達提示；模式切換本身不播出發或到站語音。
...
- **單人自動暫停（兩項預設開啟）**：「脫困失敗後暫停」與停靠／行程結束後暫停可各自關閉。普通自動接續點不暫停、不播最終抵達；明確停靠或最後完成時先停妥、播完對應語音再暫停，語音關閉或不可用時直接暫停。新啟動意圖會取消本 MOD 尚未執行的延遲暫停，不擅自解除玩家或 ESC 的暫停。僅限單一玩家的單人遊戲，多人／Host／分割畫面不生效；等待語音時世界仍運作，並非掛機保命模式。
  暫時堵路仍會繼續脫困，手動關閉不觸發暫停；解除暫停也不會自行重開自駕。
  逐點模式的每站（含最後一站）也依此設定在語音結束後暫停；純 MiniMap 手動導航不會觸發自駕暫停。
...
1. 由 MiniMap 放大鏡／`;` 開啟行程。搜尋或地圖右鍵可加到最後、指定插入位置或「先去這裡」。
2. 在行程頁選自動／逐點；要補給或下車的目標先標成停靠。新行程預設自動，舊行程維持逐點。
3. 停車後按 HUD「開始行程」。每段道路與限速剖面準備完成才接管，準備時可按「取消」。
4. 自動模式的普通點停妥後接下一點；明確停靠與逐點模式按「繼續自駕」才出發。
5. HUD 的圖標＋「自動／逐點」與行程頁是同一份設定；切換不發車、不煞車、不換目標。
6. 修改目前點或優先插隊前先停止自駕並停車；後續區段的合法編輯不會中斷本段。

HUD 會區分已停靠、已略過、下一目標與行程完成，完整內容在主鈕提示；
大字型或極窄畫面優先保留主要動作與收合入口，側翼放不下時退回上掛版面。

停止自駕或導航會交還方向盤，**不等於緊急煞車**。道路無法直達的站點保留待辦，
可手動前往或在 MiniMap 明確略過。開發用行程 API 契約見同層 MiniMap 專案的
`docs/addon-api.md`。
